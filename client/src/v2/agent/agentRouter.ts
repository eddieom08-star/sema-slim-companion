...
export function classifyIntent(text: string): Intent {
  ...
  if (has(['recipe','what should i cook','what should i eat','meal idea','suggest something','what can i make','generate a'])) return 'recipe.generate'
  if (has(['i ate','i had','i\'ve had','just ate','just had','for breakfast','for lunch','for dinner','for tea','for supper','calories in','log meal','log a meal','log food','add food','ate a ','had a ','had some'])) return 'food.search'
  if (has(['feel full','quite full','not hungry','craving','satiety','fullness','appetite level','log my appetite','doing with appetite','hunger level','log hunger','my hunger','log my hunger'])) return 'food.appetite'
  ...
  if (has([' kg','kg ',' lbs','lbs ','lbs',' pounds',' stone','stone and'])) return 'weight.log'
